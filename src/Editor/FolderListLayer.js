var FolderListLayer = cc.Layer.extend({
	ctor:function(width, height, userLevels){
		...
		for(var i=0; i<this.userLevels.length && i < 4; i++)
		{
			...
			var folderBox = new FolderBox(listWidth*.95, this.boxHeight*.9, Object.values(userLevels[i].levelTypes),Object.keys(userLevels[i].colors), "Level "+(i+1));
			folderBox.attr({
				x:0+listWidth*.025,
				y:this.height-(i+1)*(this.boxHeight) - this.boxHeight*.05*(1+(i*2)),
				anchorX:0,
				anchorY:0
			});
	...
	onTouchEnded:function(pos)
	{
		...
		{cc.log("UP ARROW YO");
			...
			{cc.log(this.topRow);
				...
				this.folderBoxes.unshift(new FolderBox(this.listWidth, this.boxHeight,  userLevels[this.topRow].levelTypes,userLevels[this.topRow].levelColors, "Level "+(this.topRow+1)));
		...
		{cc.log("DOWN ARROW YO");
			...
			{cc.log(this.topRow);
				...
				this.folderBoxes.push(new FolderBox(this.listWidth, this.boxHeight, userLevels[this.topRow+3].levelTypes,userLevels[this.topRow+3].levelColors,  "Level "+(this.topRow+3+1)));
				this.folderBoxes[3].attr({
					x:0,
					y:this.height-4*(this.boxHeight) ,
					anchorX:0,
		...
		else
		{
			...
			{*/
				...
				this.curSelected = this.topRow+rowTouched;
				this.parent.parent.parent.userLevelIndex = this.curSelected;
				...
				else if(this.parent.parent.parent.featureInProg)
				{
					this.parent.parent.parent.openQuitPopup();
					this.parent.parent.parent.quitWithoutFinishingAction = {"type":"folder","number":this.topRow+rowTouched};
				}
				else
				{
