var WorldListHeader = cc.Layer.extend({
	ctor:function(width, height, streak){
		...
		this.title = new cc.LabelTTF("World 1", "HeaderFont", Math.floor(this.height*.5));
		...
		this.subTitle = new cc.LabelTTF("Soapy Stack", "HeaderFont", Math.floor(this.height*.25));
		...
		this.giftImg.setScale(this.height*.8 / this.giftImg.height);
		this.giftImg.attr({
			x:this.width-3,
			y:this.height/2,
			anchorX:1,
		...
		//this.barDn.drawRect(cc.p(), cc.p(), cc.color(), 0, cc.color(0,0,0,0) );
