var PreChallengeLayer = cc.Layer.extend({
	ctor:function(challengeIndex, width, height){
        ...
        */
		...
		this.levelHeader = new cc.LabelTTF("Level","Arial",36);
		...
		if(DATA.streakStep == 2)
			streakNumText = "MAX";
		this.streakText = new cc.LabelTTF("Streak: "+DATA.streakStep, "Arial", 25);
		...
		this.preBoosterA.attr({
			x:this.tryAlert.x+(this.tryAlert.width*this.tryAlert.scale)+4,
			y:this.levelMovesNumber.y-(this.levelMovesNumber.height*.6),
			anchorX:0,
			anchorY:.5
		});
