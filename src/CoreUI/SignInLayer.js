var SignInLayer = cc.Layer.extend({
		...
		if(cc.sys.capabilities.hasOwnProperty('mouse')) {
			cc.eventManager.addListener({
				...
				onMouseUp:function(event)
				{
					...
					self.executeOnEnd(event.getLocation());
					return false;
				}
			}, this);
		}
			...
			cc.eventManager.addListener({
			    ...
			    onTouchEnded: function(touch, event){
				    ...
				    var locationInNode = self.convertToNodeSpace(touch.getLocation());
			    		...
			    		self.executeOnEnd(locationInNode);
	...
	executeOnEnd:function(locationInNode)
	{
		// LOG IN
	    	if(FUNCTIONS.posWithinScaled(locationInNode, this.logInButton))
	    	{
				...
				/*
					...
					*/
					DATA.initUserWebData(DATA.userID);
	    	...
	    	// NEW USER SIGNUP
	    	else if(FUNCTIONS.posWithinScaled(locationInNode, this.newUserButton))
	    	{
	    		var email = prompt("Please enter email.", "");
				var password = prompt("Password","");
