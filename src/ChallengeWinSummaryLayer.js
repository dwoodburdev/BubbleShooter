var ChallengeWinSummaryLayer = cc.Layer.extend({
	ctor:function(width, height){
		...
		this.nextButton.setScale(this.height*.1 / this.nextButton.height)
		...
		this.cardSet = {x:5,y:this.tabTitleLabel.y-this.tabTitleLabel.height-10-(this.height*.25), width:(this.width-10), height:this.height*.25};
		
		
		/*this.challengeCollectionDisplay = new ChallengeCollectionDisplayLayer(this.width*.9, this.height*.2);
   		this.challengeCollectionDisplay.attr({
   			"x":this.width*.05,
		...
		var circleY = (this.nextButton.y+(this.nextButton.height*this.nextButton.scale)) + circleR;
		//circleR -= 2;
		circleR = 20;
		// Circles' lit color
		...
		this.dn.drawDot({x:this.x+size.width/2 - circleR*2 - 5, y:circleY}, circleR, circleColor);
		
		if((DATA.streakStep == 1 && DATA.challengeTries == 0) || (DATA.streakStep == 2 && DATA.challengeTries <= 1))
			this.dn.drawDot({x:this.x+size.width/2, y:circleY}, circleR, circleColor);
		else if(DATA.streakStep > 0)
			this.dn.drawDot({x:this.x+size.width/2, y:circleY}, circleR, cc.color(100,100,100,255));
		else
			this.dn.drawDot({x:this.x+size.width/2, y:circleY}, circleR, cc.color(0,0,0,255));
			
		if(DATA.streakStep == 2 && DATA.challengeTries == 0)
			this.dn.drawDot({x:this.x+size.width/2 + circleR*2 + 5, y:circleY}, circleR, circleColor);
		else if(DATA.streakStep == 2)
			this.dn.drawDot({x:this.x+size.width/2 + circleR*2 + 5, y:circleY}, circleR, cc.color(100,100,100,255));
		else this.dn.drawDot({x:this.x+size.width/2 + circleR*2 + 5, y:circleY}, circleR, cc.color(0,0,0,255));
		
		
		
		this.streakUpAlert = new cc.Sprite(res.streak_up_alert);
		this.streakUpAlert.setScale(this.nextButton.width / this.streakUpAlert.width);
		...
		this.challengeARect = {
			x: 5,
			y: this.cardSet.y - 5 - challengeHeight,
	...
	{
		if(pos.x > this.nextButton.x && pos.x < this.nextButton.x+(this.nextButton.width*this.nextButton.scale)
		&& pos.y > this.nextButton.y && pos.y < this.nextButton.y+(this.nextButton.height*this.nextButton.scale))
		{
			return "next";
		}
		return null;
