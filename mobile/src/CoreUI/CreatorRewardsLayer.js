var CreatorRewardsLayer = cc.Layer.extend({
	ctor:function(width, height){
		...
		*/
		...
		this.boxHeight = (cc.winSize.width/24)*10;
		this.boxBorder = Math.floor((cc.winSize.width/24)*.5);
	...
	{cc.log("CREATOR LEVELS YOOOOOOOOO");
		...
		if(this.posWithin(loc, this.levelBox))
		...
		else if(this.posWithin(loc, this.leftBox))
		{
		...
		else if(this.posWithin(loc, this.rightBox))
		{
		...
		{cc.log("should find edit button...");
			if(this.posWithinScaled(loc, this.editButton))
			{
				cc.log("edit");
				return {"type":"edit","number":this.topBoxIndex};
			}
			else if(this.posWithinScaled(loc, this.testButton))
			{
				cc.log("test");
				return {"type":"test","number":this.topBoxIndex};
			}
			/*else if(this.posWithinScaled(pos, this.shareButton))
			{
				cc.log("share");
	...
	draw:function(){
		...
		var boxY = 0;
		var boxBorder = Math.floor((cc.winSize.width/24)*.5);
	...
	{
		if(a.x > b.x && a.x < b.x+b.width && a.y > b.y && a.y < b.y+b.heieght)
			return true;
		return false;
	}
