var EditorUILayer = cc.Layer.extend({
	ctor:function(w,h){
        ...
        this.backHomeButton.setScaleX(((cc.winSize.width/24)*5-1) / this.backHomeButton.width);
        this.backHomeButton.setScaleY((cc.winSize.width/24)*2 / this.backHomeButton.height)
		...
		this.saveButton.setScaleX((cc.winSize.width/24)*5 / this.saveButton.width);
		this.saveButton.setScaleY((cc.winSize.width/24)*2 / this.saveButton.height);
	...
	loadButtons:function()
	{
		...
		for(var e=0; e<emojiNames.length; e++)
		{
			for(var i=0; i<emojiNames[e].length; i++)
			{
				...
				if(img.y > (cc.winSize.width/24)*2.5)
				{
					this.addChild(img);
	...
	scrollButtonsDown:function()
	{
		...
		else
		{
			for(var i=0; i<this.imgButtons.length; i++)
			{
				...
				if(!this.buttonFlags[i] && this.imgButtons[i].y <this.height+(cc.winSize.width/24)*2 && this.imgButtons[i].y > this.height)
				{
	...
	{cc.log(this.scrollUpFlag);
		if(this.imgButtons[this.imgButtons.length-1].y+this.scrollSpeed > (cc.winSize.width/24)*4)
		{
			this.stopScrollUp();
		...
		else
		{
			...
			{
				this.imgButtons[i].attr({y:this.imgButtons[i].y+this.scrollSpeed});
				if(!this.buttonFlags[i] && this.imgButtons[i].y > (cc.winSize.width/24)*2 && this.imgButtons[i].y < (cc.winSize.width/24)*4)
				{
					this.addChild(this.imgButtons[i]);
					this.buttonFlags[i] = true;
				}
				else if(this.buttonFlags[i] && this.imgButtons[i].y > this.height+(cc.winSize.width/24)*2)
				{
					this.removeChild(this.imgButtons[i]);
					this.buttonFlags[i] = false;
				}
	...
	{
		var rowClicked = Math.floor((this.height-pos.y)/((cc.winSize.width/24)*2));
		var bulbIterClicked = Math.floor(rowClicked/2);
		if(pos.x > this.dividerX+((cc.winSize.width/24)*2) && pos.x < this.width-((cc.winSize.width/24)*2)
			&& pos.y < this.height && pos.y > 0)
		{
			var colClicked = Math.floor((pos.x - (this.dividerX+((cc.winSize.width/24)*2)))/((cc.winSize.width/24)*2));
			var bulbIterClicked = Math.floor(rowClicked/2);
			...
			if(rowClicked%2 == 0)
			{
				...
				this.editorHighlightA.setScale(((cc.winSize.width/24)*2)/this.editorHighlightA.width);
				this.editorHighlightA.attr({
					x:this.dividerX,
					y:this.height - rowClicked*(cc.winSize.width/24)*2,
					anchorX:0,
					anchorY:1
				});
			...
			newBulb.setScale((cc.winSize.width/24)*2 / newBulb.width);
			newBulb.attr({
				x:oldBulb.x,
				y:oldBulb.y,
				anchorX:0,
		...
		else if(pos.x < this.dividerX+((cc.winSize.width/24)*2) )
		{
			if(rowClicked%2 == 0)
			{
				if(this.editorHighlightA != null)
	...
	rightMenuPathTouch:function(pos)
	{
		...
		if(pos.x > this.dividerX+(cc.winSize.width/24) && pos.x < this.width-((cc.winSize.width/24)*3)
			&& pos.y < this.height && pos.y > this.height-((cc.winSize.width/24)*4))
			{
				if(this.editorHighlightA != null)
				{
					this.removeChild(this.editorHighlightA);
				...
				var rowClicked = Math.floor((this.height-pos.y)/((cc.winSize.width/24)*2));
				var colClicked = Math.floor((pos.x - (this.dividerX+((cc.winSize.width/24)*1)))/((cc.winSize.width/24)*2));
				cc.log(rowClicked + " "+colClicked);
				this.editorHighlightA = new cc.Sprite(res.thick_black_circle);
				this.editorHighlightA.setScale(((cc.winSize.width/24)*2)/this.editorHighlightA.width);
				this.editorHighlightA.attr({
					x:this.dividerX+(cc.winSize.width/24)+(colClicked*(cc.winSize.width/24)*2),
					y:this.height - rowClicked*(cc.winSize.width/24)*2,
					anchorX:0,
					anchorY:1
				});
			...
			else if(pos.x > this.dividerX+(cc.winSize.width/24)*2 && pos.x < this.width-((cc.winSize.width/24)*4)
			&& pos.y < this.height-((cc.winSize.width/24)*5) && pos.y > this.height-((cc.winSize.width/24)*11))
			{
				...
				var rowClicked = Math.floor((this.height-((cc.winSize.width/24)*5)-pos.y)/((cc.winSize.width/24)*2));
				var colClicked = Math.floor((pos.x - (this.dividerX+((cc.winSize.width/24)*2)))/((cc.winSize.width/24)*2));
				cc.log(rowClicked + " "+colClicked);
				this.editorHighlightB = new cc.Sprite(res.thick_black_circle);
				this.editorHighlightB.setScale(((cc.winSize.width/24)*2)/this.editorHighlightB.width);
				this.editorHighlightB.attr({
					x:this.dividerX+((cc.winSize.width/24)*2)+(colClicked*(cc.winSize.width/24)*2),
					y:(this.height-((cc.winSize.width/24)*5)) - rowClicked*((cc.winSize.width/24)*2),
					anchorX:0,
					anchorY:1
				});
	...
	rightMenuColorPickerTouch:function(pos)
	{
		...
		else if(pos.x > this.dividerX+((cc.winSize.width/24)*2) && pos.x < this.dividerX+((cc.winSize.width/24)*6)
			&& pos.y < this.height-((cc.winSize.width/24)*5) && pos.y > this.height-((cc.winSize.width/24)*7))
		{
			if(this.editorHighlightB != null)
			{
				this.removeChild(this.editorHighlightB);
			...
			this.editorHighlightB.attr({
				x:this.dividerX+((cc.winSize.width/24)*2)+(colClicked*(cc.winSize.width/24)*2),
				y:this.height - ((cc.winSize.width/24)*5),
				anchorX:0,
				anchorY:1
			});
			this.addChild(this.editorHighlightB);
	...
	{cc.log("resetBulbIcons");
		...
		for(var i=0; i<3; i++)
		{
			...
			headBulb.setScale((cc.winSize.width/24)*2 / headBulb.width);
			headBulb.attr({
				x:this.dividerX,
				y:this.height-(i*(cc.winSize.width/24)*4),
				anchorX:0,
				anchorY:1
			});
			...
			for(var j=0; j<6; j++)
			{
				var xOff = Math.floor(j/3)*(-1*(cc.winSize.width/24)*6) + j*((cc.winSize.width/24)*2);
				var yOff = Math.floor(j/3)*((cc.winSize.width/24)*2);
				...
				inputBulb.setScale((cc.winSize.width/24)*2 / inputBulb.width);
				inputBulb.attr({
					x:this.dividerX+(cc.winSize.width/24)*2+xOff,
					y:this.height-(i*(cc.winSize.width/24)*4)-yOff,
					anchorX:0,
					anchorY:1
				});
	...
	openPathSidemenu:function()
	{
		...
		{
			var xOff = i*((cc.winSize.width/24)*2) - Math.floor(i/3)*((cc.winSize.width/24)*6);
			var yOff = Math.floor(i/3)*(cc.winSize.width/24)*2;
			
			var colorSprite = this.getSprite(colorSpriteNames[i]);
			colorSprite.setScale((cc.winSize.width/24)*2 / colorSprite.width);
			colorSprite.attr({
				x:this.dividerX+(cc.winSize.width/24) + xOff,
				y:this.height-((cc.winSize.width/24)*2)-yOff,
				anchorX:0,
				anchorY:0
			});
		...
		{
			var xOff = i*((cc.winSize.width/24)*2) - Math.floor(i/2)*((cc.winSize.width/24)*4);
			var yOff = Math.floor(i/2)*(cc.winSize.width/24)*2;
			
			var pathSprite = this.getSprite(pathSpriteNames[i]);
			pathSprite.setScale((cc.winSize.width/24)*2 / pathSprite.width);
			pathSprite.attr({
				x:this.dividerX+((cc.winSize.width/24)*2) + xOff,
				y:this.height-((cc.winSize.width/24)*7)-yOff,
				anchorX:0,
				anchorY:0
			});
	...
	openPickerSidemenu:function(bubData)
	{
		...
		for(var i=0; i<binarySpriteNames.length; i++)
		{
			...
			binSprite.setScale((cc.winSize.width/24)*2 / binSprite.width);
			binSprite.attr({
				x:this.dividerX+((cc.winSize.width/24)*2)+i*((cc.winSize.width/24)*2),
				y:this.height-((cc.winSize.width/24)*7),
				anchorX:0,
				anchorY:0
			});
	...
	draw:function(){
		...
		this.dn.drawRect(cc.p(this.width-((cc.winSize.width/24)*2), (cc.winSize.width/24)*2),
						cc.p(this.width, this.height), cc.color(255,255,255,255),1,cc.color(0,0,0,255));
		
		if(this.sidemenuMode=="bulb")
		{
			this.dn.drawRect(cc.p(this.dividerX+(cc.winSize.width/24)*2, 0),cc.p(this.dividerX+(cc.winSize.width/24)*2, this.height),
						cc.color(255,255,255,255),1,cc.color(0,0,0,255));
						
			this.dn.drawRect(cc.p(this.dividerX, (cc.winSize.width/24)*4),cc.p(this.width-(cc.winSize.width/24)*2, (cc.winSize.width/24)*4),
						cc.color(255,255,255,255),1,cc.color(0,0,0,255));
			this.dn.drawRect(cc.p(this.dividerX, (cc.winSize.width/24)*8),cc.p(this.width-(cc.winSize.width/24)*2, (cc.winSize.width/24)*8),
						cc.color(255,255,255,255),1,cc.color(0,0,0,255));
